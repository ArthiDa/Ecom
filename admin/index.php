            ...
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                  ...
                  <?php
                  $ProductSql = "SELECT COUNT(productID) AS productlist FROM product";
                  $ProductRes = mysqli_query($conn,$ProductSql);
                  $Product = 0;
                  if($ProductRow = mysqli_fetch_assoc($ProductRes)){
                    $Product = $ProductRow['productlist'];
                  }
                  $OrderSql = "SELECT COUNT(orderID) AS orderlist FROM orders";
                  $OrderRes = mysqli_query($conn,$OrderSql);
                  $Order = 0;
                  if($OrderRow = mysqli_fetch_assoc($OrderRes)){
                    $Order = $OrderRow['orderlist'];
                  }
                  $CustomerSql = "SELECT COUNT(customerID) AS customerlist FROM customer where Status = 1";
                  $CustomerRes = mysqli_query($conn,$CustomerSql);
                  $Customer = 0;
                  if($CustomerRow = mysqli_fetch_assoc($CustomerRes)){
                    $Customer = $CustomerRow['customerlist'];
                  }
                  ?>
                  <!-- Line Chart -->
                  <div id="piechart" class="d-flex justify-content-center">
                    ...
                    <script>
                    var xValues = ["Sales", "Products", "Cutomers"];
                    var yValues = [<?php echo $Order;?>, <?php echo $Product;?>, <?php echo $Customer;?>];
                    var barColors = [
                      "#b91d47",
                      "#1e7145",
                      "#2b5797",
